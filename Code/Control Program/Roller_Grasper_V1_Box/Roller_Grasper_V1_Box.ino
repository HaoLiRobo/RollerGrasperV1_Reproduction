...
int paramSequence[5][8] = {{0,0,0,0,0,112,108,112}, {-300,-300,300,0,0,112,108,112},{0,0,0,0,0,112,108,112},{-300,-300,300,0,0,112,108,112},{0,0,0,0,0,112,108,112}};
...
void setup() {
  ...
  for(int i=0;i<3;i++)
  {
    ...
    jointInputs[1][0]= 112 * DEG2BIT;//83
    jointInputs[1][1]= 108* DEG2BIT;//85
    jointInputs[1][2]= 112* DEG2BIT;//90
      ...
      while(Serial.available()>0)          //检测串口是否接收到了数据
  {
    ...
    Serial.print("开始运行");//串口监视器输入“1”以开始运行程序
     ...
     while (Serial.available() > 0)
      ...
      if(Val==1)
      {
           if(millis()-startTime>=4000 && curr_step<5)
   {
   startTime = millis();
